...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from CircleOS Github (http://github.com/LineageOS)." % device)
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the circle snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/circle.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('CircleOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: CircleOS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "CircleOS/%s" % repo_name })
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/circle.dependencies'
    syncable_repos = []
    verify_repos = []
...
print("Repository for %s not found in the CircleOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
