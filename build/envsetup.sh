function __print_circle_functions_help() {
cat <<EOF
Additional CircleOS functions:
...
- githubremote:    Add git remote for CircleOS Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/circle-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/CircleOS/$PROJECT
...
function repopick() {
    T=$(gettop)
    $T/vendor/circle/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CIRCLE_FIXUP_COMMON_OUT ]; then
